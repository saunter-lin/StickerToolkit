...
目前版本為 **v1.3.6**。GitHub Releases 的最新正式安裝檔目前為 v1.3.6。
...
| 平台 | 支援架構 | 下載格式 |
| --- | --- | --- |
| macOS | Apple Silicon（arm64） | `StickerToolkit-v1.3.6-macOS-arm64.dmg` |
| Windows 10／11 | x64 | `StickerToolkit-v1.3.6-Windows-x64.zip` |
...
- 「預設背景色」可選米白 `#FFF8EC`、深藍 `#003366` 或自訂色；深藍適合白色、淺灰或銀灰色角色。
- 自訂色與目前預設會分別記住；切換固定色後再回到自訂，仍會恢復最後一次選擇。
- 三個選項會顯示實際色塊與 HEX 色碼，去背仍只處理與畫布外部連通的指定顏色。
- 背景避免漸層、陰影、紋理或複雜內容。
- 建議在角色及文字周圍加入白色描邊。
- PNG 容差建議 `3–5`。
