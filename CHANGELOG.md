...
- 純色背景設定新增米白 `#FFF8EC`、深藍 `#003366` 與自訂三種預設選項。
- 三個選項使用 32×16 px、具中性灰描邊的原生 Qt 色塊，並同步顯示實際 HEX 色碼。
- 使用 QSettings 分別保存目前預設與最後一次自訂色；切換固定色後再回到自訂色仍會恢復原值。

### Changed

- 背景色主要操作由自動偵測改為明確的預設選單；底層去背、容差及 LINE／WeChat 輸出流程維持不變。
- 舊版自動／手動背景色設定會安全映射至新預設，無效資料則回退至米白。
...
- Desktop GUI 新增繁體中文、简体中文與 English，可在主視窗即時切換。
- 首次啟動依系統語言選擇介面；手動選擇後使用既有 QSettings 跨平台保存。
- 靜態介面、檔案對話框、驗證、進度、結果與錯誤訊息共用具 English fallback 的 i18n 翻譯 key。
- 切換語言時保留已選圖片、平台、模式、輸出位置、背景容差與 Batch 清單順序。

### Compatibility

- 圖片處理與輸出邏輯維持不變；介面語言不影響 CLI、LINE／WeChat 規格、輸出名稱或目錄結構。
...
- Desktop 自動或手動選擇的輸出位置統一視為根目錄，實際產物集中於其下的 `output/`。
- 使用者直接選擇名為 `output` 的資料夾時會沿用該目錄，不會建立 `output/output`。
